...
bool read_gguf_string(std::ifstream &file, std::string *out) {
    ...
    std::cout << "[read_gguf_string] string size: " << size << std::dec;
    
    // Check for unreasonable string sizes
    if (size > 1000000) {  // 1MB limit
        std::cout << "❌ String size too large: " << size << std::endl;
        return false;
    }
    
    if (size > static_cast<std::uint64_t>(std::numeric_limits<std::size_t>::max())) {
        std::cout << "❌ String size exceeds size_t limit" << std::endl;
        return false;
    }
...
int main(int argc, char** argv) {
    std::cout << "=== minllama GGUF Reading Debug Tool (Fixed) ===" << std::endl;
    ...
    for (uint64_t i = 0; i < n_kv; ++i) {
        ...
        if (type_raw == 8) {  // String
            ...
            // Check string size
            if (str_size > 1000000) {
                std::cout << "❌ String too large, skipping" << std::endl;
                file.seekg(str_size, std::ios::cur);
            } else if (str_size > 0) {
