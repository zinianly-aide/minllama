...
// Create a minimal valid GGUF v3 file for testing (fixed)
int main() {
    std::cout << "Creating fixed minimal test GGUF v3 file..." << std::endl;
    
    std::ofstream file("/tmp/test_minimal_fixed.gguf", std::ios::binary);
    if (!file) {
    ...
    // Write tensor info
    // Tensor name: "token_embd.weight" (length 17, no null terminator)
    uint32_t name_len = 17;
    file.write(reinterpret_cast<const char*>(&name_len), 4);
    file.write("token_embd.weight", 17);
    ...
    // Offset: after header + KV + tensor info (24 + 8+20+4+8+5 + 4+17+4+8+4) = 72
    uint64_t offset = 72;
    ...
    std::cout << "✅ Created fixed minimal test GGUF file" << std::endl;
    std::cout << "File size: " << (72 + dummy_data.size() * 2) << " bytes" << std::endl;
